...
AI 어시스턴트는 작업 중 발생한 실수나 사용자로부터 받은 중요한 피드백을 아래에 기록하여 동일한 실수를 반복하지 않도록 합니다. **특히 명령어 실행 중 구문 오류 등이 발생할 경우, 누가 요청하지 않아도 알아서 실수를 분석하고 이곳에 기록해야 합니다.**

### [실수 기록 및 교훈]
- **PowerShell 구문 오류**: Windows 환경(PowerShell)에서는 `&&` 연산자를 지원하지 않으므로, 여러 명령어를 연속 실행할 때는 `;` 또는 별도의 명령어로 분리하여 실행해야 함. (예: `git add . ; git commit -m "..."`)
- **경로 및 환경 설정 오류**: 백그라운드 명령어 실행 시 작업 디렉토리(Cwd)와 실행 파일의 절대 경로를 명확히 지정하여 경로 미찾음 및 환경 변수 누락 문제를 예방해야 함.
- **Hook 사용 규칙**: 컴포넌트 렌더링 중 훅(Hook)의 개수가 변하거나 조건부로 호출되지 않도록 구조화에 유의해야 하며, 특히 HMR(Fast Refresh) 시 발생할 수 있는 렌더링 불일치에 주의해야 함.
- **데이터 직렬화**: Firestore 등 클라우드 DB 저장 시 Firebase User 객체와 같은 비직렬화(Non-serializable) 데이터가 포함되지 않도록 상태 추출 시 필터링을 철저히 해야 함.
